...
try:
	...
	listitem = xbmcgui.ListItem('HomeandGarden TV')
	listitem.setInfo('video', {'Title': 'HomeandGarden  TV'})
	listitem.setArt({ 'thumb': thumb_url})
	xbmc.Player().play(streaming_url, listitem)

except Exception as ex:
	message = 'Eccezione: %s' % ex
	listitem = xbmcgui.ListItem('HomeandGarden  - Errore')
	listitem.setInfo('video', {'Title': 'Errore nell\' avvio del plugin HomeandGarden '})
